...
class SiteController extends BaseController
{
		...
		if($identity) {
			...
			if(Yii::$app->params['guestUserMode'] == 'auto' && isset($session['screenData']) && isset($session['screenData']['app\models\UserAccount'])) {
				...
				if($existModel->user_name == Yii::$app->params['guestUserCredentials']['username']) {
                    ...
                    if($identity->getStartScreen() == Url::toRoute(['/site/index'])) {
                        //unset($session['screenData']);
    ...
    public function actionLogin($username = '', $password = '')
    {
		if ($username != '' && $password != '') {
			...
			$post_param = array('_csrf_render' => Yii::$app->request->csrfToken, 'LoginForm' => array('username' => $username, 'password' => $password));
			...
			$post_param = array('_csrf_render' => Yii::$app->request->csrfToken, 'LoginForm' => array('username' => $username));

			if ($model->load($post_param)) {
				$model->login();
            ...
            if(!empty($session->get('access_token')) && !empty($session->get('user_name'))) {
                unset($session['screenData']);

                $postData = [
                    "func_name" => 'login',
                    "func_param" => [
                        'ulogin' => $session->get('user_name'),
                    ]
                ];

                $user = UserAccount::getModelInstance(null, $postData);

                if (!empty($user) && Yii::$app->user->login($user, 0)) {
                    UserAccount::initSettings();

                    $identity = Yii::$app->getUser()->getIdentity();
                    return !empty($identity) ? $this->redirect($identity->getStartScreen()) : null;
                }

                return $this->redirect(Url::toRoute(['/saml/sls']));
            }

            if (isset(Yii::$app->params['guestUserMode']) && Yii::$app->params['guestUserMode'] == 'auto') {
				unset($session['screenData']);

				if(Yii::$app->params['guestUserCredentials']['username'] != '' && Yii::$app->params['guestUserCredentials']['password'] != '') {
					...
					if ($model->load($post_param) && $model->validate()) {
						...
						if (!empty($identity) && $this->isSAMLTenant()) {
                            return $this->redirect(Url::toRoute(['/saml/login']));
                        }

						return !empty($identity) ? $this->redirect($identity->getStartScreen()) : null;
					}
				} else if(Yii::$app->params['guestUserCredentials']['username'] != '' && Yii::$app->params['guestUserCredentials']['password'] == '') {
					$post_param = array('_csrf_render' => Yii::$app->request->csrfToken, 'LoginForm' => array('username' => Yii::$app->params['guestUserCredentials']['username']));
		...
		if ($model->load($post_param) && $model->validate() && AuthHelper::init()) {
            ...
            if (AuthHelper::getStatus() == AuthHelper::STATUS_PROGRESS) {
				...
				if($identity)
					...
					return 'incorrect_username_and_password';
            } else {
				return 'incorrect_username_and_password';
			}
		} else {
		...
		if ($action == 'check_code') {
			if (AuthenticationModel::checkAuthTypeCode(AuthHelper::AUTH_TYPE_EMAIL, $confirmation_code)) {
				...
				if (AuthHelper::getStatus() == AuthHelper::STATUS_COMPLETED) {
					...
					return AuthHelper::getCurrentType();
				}
			} else {
	...
	public function actionCheckSms($action = 'get_code', $confirmation_code = '')
    {
		...
		if ($action == 'check_code') {
			if (AuthenticationModel::checkAuthTypeCode(AuthHelper::AUTH_TYPE_SMS, $confirmation_code)) {
				...
				if (AuthHelper::getStatus() == AuthHelper::STATUS_COMPLETED) {
					...
					//$identity = Yii::$app->getUser()->getIdentity();
					//echo '<pre> $identity :: '; print_r($identity);
	...
	public function actionResetPassword() {
		...
		$response = AuthenticationModel::resetPassword(Yii::$app->request->post('account_name'), Yii::$app->request->post('email'), Yii::$app->request->post('reset_password_template'));

		if(!empty($response['resultbody'])) {
			if($response['resultbody']['requestresult'] == 'successfully') {
				$result['action'] = 'success';
				$result['message'] = $response['resultbody']['extendedinfo'];
			} else if($response['resultbody']['requestresult'] == 'unsuccessfully') {
				$result['action'] = 'error';
				$result['message'] = $response['resultbody']['extendedinfo'];
			}
		}
	...
	public function actionRegistration() {
		...
		$response = AuthenticationModel::registration(Yii::$app->request->post());
    ...
    public function actionRenderTab()
    {
		...
		//echo '<pre> Yii::$app->session'; print_r(Yii::$app->session['tabData']); die;
        ...
        $screenName = Yii::$app->request->post('screenName', false);
		$approvedMessagesCode = Yii::$app->request->post('approvedMessagesCode', []);
        $tableSectionFilterArray = Yii::$app->request->post('tableSectionSelectedFilterArray', false);
        $inputValues = Yii::$app->request->post('input_values', false);
		...
		$screenData = Yii::$app->session['tabData']->tplData;

		//echo '<pre> $screenData :: '; print_r($screenData); die;
		if($screenName != '' && $screenName != null && empty(Yii::$app->session['tabData']->getSelectScreen($tid))) {
            $tabModel = Screen::getData(['screen_name' => [$screenName]]);
            Yii::$app->session['tabData'] = $tabModel;

			if($button_action == '' && $button_action == null) {
				$config = $tabModel->getSearchConfig();
                if (!empty($config)) {
                    SearchWidget::widget(['config' => $config]);
                }
			}
		}
        ...
        if ($selfTab = Yii::$app->session['tabData']->getSelectScreen($tid)) {
            ...
            if ($mode == RenderTabHelper::MODE_EXECUTE) {
				...
				$result = Screen::execute($selfTab, $id, $field_val, $field_list_json, $lastFoundData, $approvedMessagesCode, $screenData);

                if ($this->isClientMessage($result)) {
                    return [
                ...
                return $this->renderAjax('element_tab', [
                    ...
                    'tableSectionFilterArray' => $tableSectionFilterArray,
    ...
    public function actionCustomExecute()
    {
		...
		$execute = Yii::$app->request->post('execute', []);
        $post = Yii::$app->request->post('post', []);
		$lastFoundData = Yii::$app->request->post('lastFoundData', []);
		$row_data = Yii::$app->request->post('row_data', []);
		$pk_key = Yii::$app->request->post('pk_key', []);
		$approvedMessagesCode = Yii::$app->request->post('approvedMessagesCode', []);
        ...
        if (!empty($row_data)) {
            if (!empty($pre)) {
                ...
                $pre = (!empty($pre)) ? (array)$pre : [];
            }

            if (!empty($execute)) {
                $execute = json_decode($execute, true);
                $execute = (!empty($execute)) ? (array)$execute : [];
            }
        ...
        if ($selfTab = Yii::$app->session['tabData']->getSelectScreen($tid)) {
			$result = Screen::executeCustom($id, $selfTab, $getFunction, $pre, $execute, $post, $lastFoundData, $row_data, $approvedMessagesCode, $pk_key);
    ...
    public function actionGetLoadData()
    {
		...
		//echo '<pre> $configCache :: '; print_r($configCache); die;
        ...
        if (!empty($configCache[$post['library']]['default']) || !empty($configCache[$post['library']]['custom'])) {
            ...
            if (!empty($configCache[$post['library']]['custom'])) {
				...
				//echo '<pre> $tabData["tpl"] :: '; print_r($tabData['tpl']); die;

                $additionalParam = [];

                if (!empty($tabData['tpl']->screen_extensions['inquire']['pre'])) {
					$pre_arr = array();

					foreach($tabData['tpl']->screen_extensions['inquire']['pre'] as $pre_key => $pre) {
                ...
                if (!empty($tabData['tpl']->screen_extensions['inquire']['post'])) {
					$post_arr = array();

					foreach($tabData['tpl']->screen_extensions['inquire']['post'] as $post_key => $temp_post) {
						$post_arr[$post_key]['id'] = $temp_post['extension_id'];
						$post_arr[$post_key]['signature'] = $temp_post['extension_lib'].';'.$temp_post['extension'].';';
					}
				...
				//echo '<pre> $additionalParam :: '; print_r($additionalParam); die;
    ...
    public function actionCreateData()
    {
		...
		if ($result = CommandData::insert($recordData, $recordSubData, $approvedMessagesCode)) {
            if ($this->isClientMessage($result)) {
                return [
                    'message' => $result['message'],
                    'message_code' => $result['message_code'],
    ...
    public function actionSaveData()
    {
		...
		$updateSubData->recordManager->setGroupingFuncType(RecordManager::ITEM_ATTR_UPDATE_FUNC);
		...
		if ($result = CommandData::update($recordData, $updateSubData, $insertSubData, $deleteSubData, $approvedMessagesCode)) {
            if ($this->isClientMessage($result)) {
                return [
                    'message' => $result['message'],
                    'message_code' => $result['message_code'],
	...
	public function actionExecuteData()
    {
		...
		if ($result = CommandData::execute($recordData, $updateSubData, $insertSubData, $deleteSubData, $approvedMessagesCode)) {
            if ($this->isClientMessage($result)) {
                return [
                    'message' => $result['message'],
                    'message_code' => $result['message_code'],
    ...
    public function actionDeleteData()
    {
		...
		//echo '<pre> $functionInfo :: '; print_r($functionInfo);
		...
		if ($result = CommandData::delete($recordManager, $functionInfo, $approvedMessagesCode)) {
            if ($this->isClientMessage($result)) {
                return [
                    'message' => $result['message'],
                    'message_code' => $result['message_code'],
    ...
    public function actionGetSubData()
    {
        ...
        $aliasFrameworkInfo = Yii::$app->request->post('aliasFrameworkInfo', false);
        $lastFoundData = Yii::$app->request->post('lastFoundData', false);
        ...
        if (!empty($lib) && !empty($page)) {
            $response->data = RenderGridWidget::widget([
                ...
                'tid' => $tid,
                'mode' => $mode,
				'aliasFrameworkInfo' => $aliasFrameworkInfo,
				'FoundData' => $lastFoundData
            ]);
	...
	private function isClientMessage($result)
    {
        return is_array($result) && !empty($result['message']) && !empty($result['message_code']) && !empty($result['message_type']);
    }

    private function isSAMLTenant()
    {
        $identity = Yii::$app->getUser()->getIdentity();
        $tenantList = $identity->tenant_list;
        $result = false;

        if (strpos($identity->auth_required, AuthHelper::AUTH_TYPE_SAML) === false) return false;
