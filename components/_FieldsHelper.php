...
class _FieldsHelper extends Widget
{
    ...
    public $identify_verification;
    public $account_protection;
    public $screen_self_registration_primary_table;
...
//                return Html::tag('i', '(access denied)', ['style' => ['color' => 'red']]);
    ...
    private function getFieldName()
    {
		if(isset($this->dataField) && $this->dataField !== '')
			...
			if(isset($this->config['field_identifier']) && $this->config['field_identifier'] != '')
				if(RenderTabHelper::$search_screen == 1)
    ...
    private function getBasicStyles()
    {
        ...
        if (!empty($this->config['field_bg_color'])) {
            ...
            $option['data-background-color'] = $this->config['field_bg_color'];
        }
    ...
    {
		//echo 'in getFieldOptions';
		//echo 'layout_type :: '. $this->layout_type;
        ...
        if (!empty($this->config['field_width_type'])) {
            if ($this->config['field_width_type'] === self::FIELD_WIDTH_TYPE_LENGTH) {
				...
				if($this->mode == '' && $this->layout_type == 'grid')
					Html::addCssStyle($options, ['width' => $this->config['field_width_value']]);
				else if($this->mode == '' && $this->layout_type == 'list')
					Html::addCssStyle($options, ['width' => $this->config['field_width_value']]);
				else if($this->mode != '' && $this->layout_type == 'list')
					Html::addCssStyle($options, ['width' => $this->config['field_width_value']]);
            }
		...
		if(isset($this->config['tab_order']) && isset($this->config['tab_index'])) {
			$options['tabindex'] = $this->config['tab_index'];
		} else {
			$options['tabindex'] = -1;
		}
    ...
    private function getSelectField(array $options, $type)
	{
		...
		//echo '$type :: ' . $type;
		//echo 'self::TYPE_MULTI_SELECT :: ' . self::TYPE_MULTI_SELECT;
		//echo '$this->config[num_rows] :: ' . $this->config['num_rows'];

		//echo '<pre> $options :: '; print_r($options);

		if($type == self::TYPE_MULTI_SELECT) {
			if (!empty($this->config['num_rows']) && $this->config['num_rows'] > 1) {
				$options['multiple'] = 'multiple';
				$options['size'] = (!empty($this->config['num_rows'])) ? $this->config['num_rows'] : 1;
			}
		} else if(isset($this->config['multi-select-type']) && $this->config['multi-select-type'] == self::PROPERTY_TRUE) {
			$options['size'] = 1;
			$options['multiple'] = 'multiple';
		} else {
			...
			if($type === self::TYPE_SELECT) {
				...
				/*if($this->getFieldName() == 'Array.JuneTest5.Country' && in_array($this->mode, [RenderTabHelper::MODE_EDIT, RenderTabHelper::MODE_INSERT]) && $this->layout_table == 'TABLE') {
					//Html::addCssClass($options, 'common-table-country-list-field');
				}*/
        ...
        $options = $this->addJsEventAndClass($options);
		...
		if(!empty($this->config['list_name'])) {
			$groupAreas = explode(';', Yii::$app->getUser()->getIdentity()->group_area);
			//echo '<pre> $groupAreas :: '; print_r($groupAreas);
			...
			if(!empty($select_field_options)) {
				foreach($select_field_options as $key => $select_field_option) {
					$user_groups = explode(',', $select_field_option['groups']);
					//echo '<pre> $user_groups :: '; print_r($user_groups);

					if(!empty($user_groups[0])) {
						foreach($user_groups as $user_group) {
							if(in_array($user_group, $groupAreas)) {
		...
		if(($type == self::TYPE_MULTI_SELECT) || (isset($this->config['multi-select-type']) && $this->config['multi-select-type'] == self::PROPERTY_TRUE))
			$value = explode(self::MULTI_SELECT_DELIMITER, $this->value);
		else
			$value = $this->value;
		...
		if ($this->isDisabledField()) {
			if($type == self::TYPE_MULTI_SELECT && (!empty($this->config['num_rows']) && $this->config['num_rows'] > 1)) {
				$input = Html::dropDownList($this->getFieldName(), $value, $items, $options);
			} else if(isset($this->config['multi-select-type']) && $this->config['multi-select-type'] == self::PROPERTY_TRUE) {
				$result = array_intersect($items, $value);
				...
				if(empty($items) && $this->value != '')
					$input = Html::dropDownList($this->getFieldName(), $value, array($this->value => $this->value), $options);
				else
					$input = Html::dropDownList($this->getFieldName(), $value, $items, $options);
			}
		} else {
			if($type == self::TYPE_MULTI_SELECT && (!empty($this->config['num_rows']) && $this->config['num_rows'] > 1)) {
				...
				$options['data-selected-text-format'] = 'count > 3';
		...
		else if($this->mode != '' && $this->layout_type == 'grid')
			return Html::tag('div', $input, ['class' => 'input-group']);
		else
			return $input;
	}
    ...
    private function getCheckboxField(array $options)
    {
        ...
        $value = (int)!(empty($this->value) || $this->value == 'false');
    ...
    private function getDateTimePicker(array $options)
    {
		...
		if(isset($this->config['use_current_date_time']))
			$options['data']['use-current-date-time'] = $this->config['use_current_date_time'];

        if (empty($this->value) && empty($this->config['allow_empty']) && (is_null($this->dataId) || $this->dataId == -1)) {
            $this->value = date("Y-m-d H:i:s");
        }

		if (empty($this->value) && isset($this->config['use_current_date_time']) && empty($this->config['use_current_date_time'])) {
            $this->value = date("Y-m-d H:i:s");
        }
    ...
    {
		//echo 'in getNumericField';
		//echo '<pre> $this->config :: '; print_r($this->config);

        Html::addCssClass($options, 'numeric-input');

        if (isset($this->config['allow-negative']) && !empty($this->config['allow-negative'])) {
            Html::addCssClass($options, 'allow-negative');
        }
		...
		if(isset($this->config['refresh_base_page_when_Exit']) && $this->config['refresh_base_page_when_Exit'] == 'Y')
			$field_refresh_base_page_when_exit = 1;
		else
			$field_refresh_base_page_when_exit = 0;
        ...
        if($this->config['link_type'] == 'Y' && $this->mode == '') {
            if($this->value != '') {
                ...
                if(isset($this->config['type-link']) && ($this->config['type-link'] == '_blank_bypass' || $this->config['type-link'] == '_modal_bypass')) {
					if($this->config['link_type_url'] != '' && isset($this->config['bypass_field_pass_through_param']) && !empty($this->config['bypass_field_pass_through_param'] != '')) {
						$string = '';

						foreach($this->config['bypass_field_pass_through_param'] as $pass_key => $pass) {
							$explode_pass_through_param = explode('.', $pass);

							$string .= $explode_pass_through_param[2].'='.$this->config['bypass_field_pass_through_param_val'][$pass_key].'&';
						}

						$string = rtrim($string, '&');

						//echo $this->config['link_type_url'];

						$url = $this->config['link_type_url'].'?'.$string;
					} else if($this->config['link_type_url'] != '') {
						$url = [$this->config['link_type_url'],
							'isFrame' => $this->config['type-link'] == '_bypass_modal'
						];
					}

					if($this->config['type-link'] == '_blank_bypass')
						$target = '_blank';
					else if($this->config['type-link'] == '_modal_bypass')
						$target = '_modal';

					//echo '<pre> $url :: '; print_r($url);

					$input = Html::a(\yii\bootstrap\Html::icon('link') . ' ' . $value, $url, ['data-field-refresh-base-page-when-exit' => $field_refresh_base_page_when_exit, 'target' => $target, 'class' => 'field-custom-link']);
				} else {
					$url = ['/screen/index',
					...
					$input = Html::a(\yii\bootstrap\Html::icon('link') . ' ' . $value, $url, ['data-field-refresh-base-page-when-exit' => $field_refresh_base_page_when_exit, 'target' => $this->config['type-link'], 'class' => 'field-custom-link']);
				}
    ...
    private function getTextField(array $options, $formatType = null)
    {
		...
		//echo $this->mode;
		//echo $this->layout_type;

		//echo '<pre>'; print_r($this->config);
		//echo '<pre> $this->tmpData :: '; print_r($this->tmpData);
		//echo $this->config['field_status'];

		/*if(isset(RenderTabHelper::$identify_verification))
			echo RenderTabHelper::$identify_verification;

		if(isset(RenderTabHelper::$account_protection))
			echo RenderTabHelper::$account_protection;*/
		...
		if (isset($this->config['field_value1']) && $this->config['field_value1'] != '')
			$field_name = $this->config['field_value1'];
		else
			$field_name = $this->getFieldName();

		if (isset($this->config['field_value1_helper']) && $this->config['field_value1_helper'] != '')
        ...
        if($this->config['link_type'] == 'Y' && $this->mode == '') {
            if($this->value != '') {
				...
				if(isset($this->config['type-link']) && ($this->config['type-link'] == '_blank_bypass' || $this->config['type-link'] == '_modal_bypass')) {
					if($this->config['link_type_url'] != '' && isset($this->config['bypass_field_pass_through_param']) && !empty($this->config['bypass_field_pass_through_param'] != '')) {
						...
						foreach($this->config['bypass_field_pass_through_param'] as $pass_key => $pass) {
							if(array_key_exists($pass, $this->tmpData))
								$string .= $this->config['bypass_field_pass_through_param_val'][$pass_key].'='.$this->tmpData[$pass].'&';
						}
				...
				if($formatType == _FormattedHelper::PHONE_TEXT_FORMAT) {
					Html::addCssClass($options, 'phone-input');
					$value = $this->Formatted->run($this->value, _FormattedHelper::PHONE_TEXT_FORMAT);
				}

				$inputType = 'hidden';
				$input .= Html::input($inputType, $field_name, $value, $options);

                if(isset($this->config['data_field']) && $this->config['data_field'] != '')
                    return Html::tag('div', $input, ['class' => 'input-group', 'style' => 'background-color: #eee; padding: 6px; border-radius: 4px;']);
                else
                    return Html::tag('div', $input, ['class' => 'input-group', 'style' => 'padding: 6px; border-radius: 4px;']);
            } else {
                return '';
            }
        } else {
			if((!empty($this->config['custom_query_pk']) && $this->config['custom_query_pk'] != '') && (!empty($this->config['custom_query_param']) && $this->config['custom_query_param'] != '')) {
				...
				if(isset($this->config['field_status']) && $this->config['field_status'] == 'self_registration_identify_verification_section_field' && isset(RenderTabHelper::$identify_verification) && RenderTabHelper::$identify_verification == 1) {
					Html::addCssClass($options, 'self_registration_identify_verification_section_fields');

					$options['data-self_registration_identify_verification_section_fields'] = 1;

					if(isset($this->config['security_filter1']))
						$options['data-security-filter1'] = $this->config['security_filter1'];
					else
						$options['data-security-filter1'] = '';

					if(isset($this->config['security_filter2']))
						$options['data-security-filter2'] = $this->config['security_filter2'];
					else
						$options['data-security-filter2'] = '';

					if(isset($this->config['security_filter3']))
						$options['data-security-filter3'] = $this->config['security_filter3'];
					else
						$options['data-security-filter3'] = '';

					if(isset($this->config['self_registration_database_field']))
						$options['data-self_registration_database_field'] = $this->config['self_registration_database_field'];
					else
						$options['data-self_registration_database_field'] = '';
				}

				if(isset($this->config['field_status']) && $this->config['field_status'] == 'self_registration_user_details_section_field') {
					Html::addCssClass($options, 'self_registration_user_details_section_fields');

					$options['data-self_registration_user_details_section_fields'] = 1;
				}

				if(isset($this->config['field_status']) && $this->config['field_status'] == 'self_registration_account_protection_section_field'&& isset(RenderTabHelper::$account_protection) && RenderTabHelper::$account_protection == 1)
					$options['data-self_registration_account_protection_section_field'] = 1;

				if(isset($this->config['security_question']) && $this->config['security_question'] != '') {
					$value = $this->config['security_question'];

					Html::addCssClass($options, 'self_registration_account_protection_section_question_fields');
				}
    ...
    private function getTextAreaField(array $options)
    {
        ...
        if($this->config['link_type'] == 'Y' && $this->mode == '') {
			...
			if((!empty($this->config['custom_query_pk']) && $this->config['custom_query_pk'] != '') && (!empty($this->config['custom_query_param']) && $this->config['custom_query_param'] != '')) {
				...
				$options['data-search-id'] = 'inline-search-'.$id;
				$options['data-display'] = $name;
				$options['data-search-limit'] = CommandData::SEARCH_LIMIT;
				$options['data-custom-query-pk'] = $this->config['custom_query_pk'];
				$options['data-custom-query-param'] = $this->config['custom_query_param'];

				Html::addCssClass($options, 'common-inline-search-textarea-class');
    ...
    private function getDocumentField(array $options)
    {
        ...
        if(!($accessRight == DocumentGroup::ACCESS_RIGHT_FULL || $accessRight == DocumentGroup::ACCESS_RIGHT_ADMIN)) {
            return Html::tag('div', 'Access denied', ['style' => 'color: red; padding: 7px 0 0 0;']);
        }
    ...
    private function getDataListRelationField($options) {
		...
		//echo '<pre> $value1 :: '; print_r($value1);
		...
		if(isset($this->config['multi-select-type']) && $this->config['multi-select-type'] == self::PROPERTY_TRUE) {
			...
			return Html::dropDownList($this->getFieldName(), $value1, [$this->value => $this->value], $options);
		}
    }
    ...
    private function getButtonField($options) {
        ...
        $options['data-btn-action'] = $this->config['button_action'];

		if(isset($this->config['hide_btn']) && $this->config['hide_btn'] == 'Y')
        ...
        if ($this->config['button_action']) {
			if ($this->config['button_action'] == 'login') {
				Html::removeCssClass($options, 'screen-btn-custom-action');
				Html::addCssClass($options, 'common-login-btn-class');
			}

			if ($this->config['button_action'] == 'password-reset-submit' || $this->config['button_action'] == 'registration-submit')
				Html::removeCssClass($options, 'screen-btn-custom-action');

            if ($this->config['button_action'] == 'report') {
				//Added to show progress status on button itself
				Html::addCssClass($options, 'progress-button');
				$options['data-loading'] = 'Generating report...';
				$options['data-finished'] = $this->config['value'];
				...
				if(isset($this->config['report_pass_through_input_no']))
					$options['data-report-pass-through-input-no'] = $this->config['report_pass_through_input_no'];
            }

            if ($this->config['button_action'] == 'execute') {
				if($this->mode == RenderTabHelper::MODE_EDIT)
					$options['disabled'] = true;

                /*if ($customFunctions = Json::decode($this->config['execute_function_custom'])) {
                    $customFunctions = explode(';', $customFunctions);
                    if (sizeof($customFunctions) > 1) {
                        ...
                        $this->view->registerJs("$('#{$this->config['identifier']}').on('click', function () {
				...
				if(
				    !empty($this->config['table_row_execute_function_pre'])
                    || !empty($this->config['table_row_execute_function_execute'])
                    || !empty($this->config['table_row_execute_function_post'])
                ) {
					Html::removeCssClass($options, 'screen-btn-custom-action');
					Html::addCssClass($options, 'common-extensions-functions-class');

					if(!empty($this->config['table_row_execute_function_pre']))
						$options['data-execute-function-pre'] = $this->config['table_row_execute_function_pre'];

					if(!empty($this->config['table_row_execute_function_execute']))
						$options['data-execute-function-execute'] = $this->config['table_row_execute_function_execute'];

					if(!empty($this->config['table_row_execute_function_post']))
						$options['data-execute-function-post'] = $this->config['table_row_execute_function_post'];

					$subId = ($this->isGridField) ? "{id: '$this->dataId'}" : "null";

					$options['data-sub-id'] = null;
				} else {
					$preFunction = !empty($this->config['execute_function_pre']) ? $this->config['execute_function_pre'] : '{}';
					$executeFunction = !empty($this->config['execute_function_execute']) ? $this->config['execute_function_execute'] : '{}';
					$postFunction = !empty($this->config['execute_function_post']) ? $this->config['execute_function_post'] : '{}';

					$subId = ($this->isGridField) ? "{id: '$this->dataId'}" : "null";

					//$getFunction = 'GetList_'.$this->primary_table;
					$getFunction = 'Search_'.$this->primary_table;

					$this->view->registerJs("$('#{$this->config['identifier']}').on('click', function () {
						common.setLastClickIdentifier('#{$this->config['identifier']}');
						common.triggerExecute('$getFunction', $preFunction, $executeFunction, $postFunction, $subId);
					});");
				}
            } else {
                $this->view->registerJs("
					$('#{$this->config['identifier']}').on('click', function (event) {
						common.setLastClickIdentifier('#{$this->config['identifier']}');
						common.triggerAction('{$this->config['button_action']}', event.target)});
				");
            }

			if($this->config['button_action'] == 'document') {
				//echo '<pre>'; print_r($this->tmpData);
				...
				if(!empty($this->tmpData)) {
					...
					//echo '<pre> document_category_list :: '; print_r($document_category_list);

					if(!empty($this->config['field_btn_document_category'])) {
						//echo '<pre> field_btn_document_category :: '; print_r($this->config['field_btn_document_category'][0]);
						//echo '<pre> $this->tmpData :: '; print_r($this->tmpData);
						...
						foreach($document_category_list as $key => $val) {
							...
							if(in_array($val['category'], $this->config['field_btn_document_category'][0])) {
								//echo 'key_part_1 :: ' . $val['key_part_1'];
								//echo $this->tmpData[$val['key_part_1']];
						...
						//echo '<pre> selected_document_category :: '; print_r($selected_document_category);
	...
	private function getImageField($options) {
		...
		if(isset($this->config['tab_order'])) {
			$tabindex = $this->config['tab_index'];
			$link_options['tabindex'] = $this->config['tab_index'];
		} else {
			$tabindex = '-1';
		...
		if(isset($this->config['image_identifier']) && $this->config['image_identifier'] != '') {
			...
			if(isset($this->config['image_action']))
        ...
        if (isset($image_action) && $image_action) {
            ...
            if ($image_action == 'execute') {
                ...
                if(
					...
					$this->view->registerJs("$('#{$image_identifier}').on('click', function () {
						common.setLastClickIdentifier('#{$image_identifier}');
						common.triggerExecute('$getFunction', $preFunction, $executeFunction, $postFunction, $subId);
					});");
				}
		...
		if(isset($this->config['image_name']) && $this->config['image_name'] != '') {
			...
			if(isset($this->config['allow_full_size_image']) && $this->config['allow_full_size_image'] == 'Y')
				return Html::a(Html::img("data:image/jpeg;base64,$image_name", $options), "javascript:void(0);", ["class" => "common_image_button_class", "id" => $image_identifier, 'tabindex' => $tabindex]);
			else
		...
		if($this->mode === RenderTabHelper::MODE_EDIT || $this->mode === RenderTabHelper::MODE_INSERT || $this->mode === RenderTabHelper::MODE_COPY) {
			...
			if (isset($image_action) && $image_action) {
				...
				if ($image_action == 'execute') {
					if($this->mode == RenderTabHelper::MODE_EDIT)
						...
						if((isset($this->config['table_row_execute_function_pre']) && $this->config['table_row_execute_function_pre'] != '') || (isset($this->config['table_row_execute_function_execute']) && $this->config['table_row_execute_function_execute'] != '') || (isset($this->config['table_row_execute_function_post']) && $this->config['table_row_execute_function_post'] != '')) {
						Html::removeCssClass($options, 'screen-btn-custom-action');
						Html::addCssClass($options, 'common-extensions-functions-class');

						if($this->config['table_row_execute_function_pre'] != '')
							$options['data-execute-function-pre'] = $this->config['table_row_execute_function_pre'];

						if($this->config['table_row_execute_function_execute'] != '')
							$options['data-execute-function-execute'] = $this->config['table_row_execute_function_execute'];

						if($this->config['table_row_execute_function_post'] != '')
							$options['data-execute-function-post'] = $this->config['table_row_execute_function_post'];
			...
			if(isset($this->config['dimage_identifier']) && $this->config['dimage_identifier'] != '') {
				...
				if($this->value != '') {
					...
					if(isset($this->config['allow_full_size_image']) && $this->config['allow_full_size_image'] == 'Y')
						return Html::a(Html::img("data:image/jpeg;base64,$image_name", $options), "javascript:void(0);", ["class" => "common_image_button_class", "tabindex" => $tabindex]);
					else
						return Html::a(Html::img("data:image/jpeg;base64,$image_name", $options), "javascript:void(0);", $link_options);
				} else {
    ...
    private function getField()
    {
        ...
        if ($this->mode === RenderTabHelper::MODE_COPY &&
            ...
            !in_array($this->config['field_type'], [
                ...
                RenderTabHelper::SECTION_TYPE_CHART_SCATTER_WITH_LINEAR_REGRESSION,
                RenderTabHelper::SECTION_TYPE_CHART_TIME_SERIES,
    ...
    private function getFileData()
    {
        if (empty($this->config['is_viewer'])) {
			...
			if ($accessRight != DocumentGroup::ACCESS_RIGHT_ADMIN && $accessRight != DocumentGroup::ACCESS_RIGHT_FULL && $accessRight != DocumentGroup::ACCESS_RIGHT_READ) {
                return Html::tag('div', 'Access denied', ['style' => 'color: red; padding: 7px 0 0 0;']);
            }
        }
    ...
    private function getFormattedData()
    {
        ...
        if ($this->config['field_type'] == self::TYPE_SELECT) {
            ...
            $data =  Html::a(\yii\bootstrap\Html::icon('link') . ' ' . $this->value, $url, [
				'data-field-refresh-base-page-when-exit' => $field_refresh_base_page_when_exit,
            ...
            if (empty($this->mode)) $options['readonly'] = true;
            $hiddenText = Html::tag('div', $this->value, ['class' => 'hidden-date-picker-text hide']);
            return $hiddenText . $this->getDatePicker($options);
        } elseif (($this->config['field_type'] == self::TYPE_TEXT || $this->config['field_type'] == self::TYPE_WITH_DEPENDENT_FIELD) && $this->config['format_type'] == _FormattedHelper::DATE_TIME_TEXT_FORMAT) {
            if (empty($this->mode)) $options['readonly'] = true;
            $hiddenText = Html::tag('div', $this->value, ['class' => 'hidden-date-picker-text hide']);
            return $hiddenText . $this->getDateTimePicker($options);
            ...
            if (empty($this->value) && !empty($this->config['allow_empty'])) {
                ...
                if($this->config['link_type'] == 'Y' && $this->mode == '') {
                    ...
                    if($this->config['link_type'] == 'N' && $this->mode == '') {
						if(strpos($this->dataField, 'image') !== false && !empty($this->layout_config->textarea)) {
							//echo '<pre>'; print_r($this->layout_config);
            ...
            if($this->config['link_type'] == 'Y' && $this->mode == '') {
                if($this->value != '') {
                    if($this->config['field_pass_through_link'] != '') {
						...
						//echo '$implode_dataField :: ' . $implode_dataField;

						//echo '<pre> $this->tmpData :: '; print_r($this->tmpData);
						//echo '<pre> $implode_dataField :: '; print_r($implode_dataField);
                    ...
                    $input = Html::a(\yii\bootstrap\Html::icon('link') . ' ' . $this->value, $url, ['data-field-refresh-base-page-when-exit' => $field_refresh_base_page_when_exit, 'target' => $this->config['type-link'], 'class' => 'field-custom-link']);
		...
		if($this->mode == '' && $this->mode == null && $this->isGridField)
			return $data;
		else
    ...
    private function addJsEventAndClass($options)
    {

        $options = $this->addAttributeDataJsEvent($options);
        if (
            !empty($this->config['js_event_edit'])
            || !empty($this->config['js_event_insert'])
            || !empty($this->config['js_event_change'])
            || !empty($this->config['js_event_onfocus'])
        ) {
            $options = $this->addCssClass($options);
        }

        return $options;
    }

    private function addAttributeDataJsEvent($options)
    {
        if (!empty($this->config['js_event_edit'])) $options['data-js-event-edit'] = $this->config['js_event_edit'];
        if (!empty($this->config['js_event_insert'])) $options['data-js-event-insert'] = $this->config['js_event_insert'];
        if (!empty($this->config['js_event_change'])) $options['data-js-event-change'] = $this->config['js_event_change'];
        if (!empty($this->config['js_event_onfocus'])) $options['data-js-event-onfocus'] = $this->config['js_event_onfocus'];

        return $options;
    }

    private function addCssClass($options)
    {
        $dataField = str_replace([':', ',', '.', ' '], '-', $this->getFieldName());
