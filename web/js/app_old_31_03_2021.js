...
var commonApp = function () {
    ...
    this.bindLoadEvents = function () {
		...
		$(window).resize(function() {
			...
			if(screen.width == window.innerWidth) {
				...
				if($('#workflow-tracker-diagram-'+field_id+ ' .btn-breadcrumb').outerWidth() <= 1000)
					$('#workflow-tracker-diagram-'+field_id+ ' .btn-breadcrumb').css('height', '36px');
				else
					$('#workflow-tracker-diagram-'+field_id+ ' .btn-breadcrumb').css('height', '55px');
        ...
        $(document)
            ...
            .on('change', '.tab-content input:not(.form-control-grid, .common-insert-row-class), .tab-content select:not(.form-control-grid), .tab-content textarea:not(.form-control-grid)', function () {
                ...
                var me = $(this);
                $('[name="' + me.attr('name') + '"]:not(.search-field, .common-insert-row-class)').val(me.val());
            })
            ...
            .on('hidden.bs.modal', '#screen-modal' , function () {
				...
				if(me.isModalData != undefined && me.isModalData != null && me.isModalData != '' && me.isModalData == 1) {
					location.reload();
				} else {
	...
	this.shallowEqual = function (obj1, temp_index, temp_pk_alias_arr) {
		...
		for(var i = 0; i < temp_pk_alias_arr.length; i++) {
			const obj2Length = Object.keys(temp_pk_alias_arr[i]).length;
    ...
    this.updateWorkflowIconContainer = function (flowStepId, activePK) {
		...
		/*var FlowId2 = $('.screen-tab.btn.active').data('flow-id');

		console.log('FlowId2');
		console.log(FlowId2);

		if(FlowId2.length) {
			FlowId2.forEach(function (item2) {
				console.log('item2');
				console.log(item2);

				var workflow_screen_current_step = $('#screen_workflow_current_step_'+'_'+item2).val();

				console.log('workflow_screen_current_step');
				console.log(workflow_screen_current_step);

				if(workflow_screen_current_step != '' && workflow_screen_current_step != undefined) {
					var temp_flow_id = workflow_screen_current_step.FlowId;
					var temp_current_step_id = workflow_screen_current_step.CurrentStepId;

					$.ajax({
						type: 'POST',
						cache: false,
						url: workflow_url,
						data: {workflow_id: temp_flow_id},
						success: function (response) {
							console.log('response');
							console.log(response);
									...
									if(flow_item.Steps[0].length) {
										...
										if(me.workflowSteps.length > 0) {
											me.workflowSteps.forEach(function (item) {
												...
												if(item.StepUuid == temp_current_step_id)
													activeClass = 'info';
												else
													activeClass = 'default';
	...
	this.arrangeWorkflowSteps = function(steps) {
		...
		steps.forEach(function (item, index) {
			...
			me.workflowSteps.push(steps_obj);
		});
	}
    ...
    this.setActiveId = function (id, foundData, isReload, section_to_refresh='', section_depth_value='', field_id='', button_action='') {
		...
		var url = this.getWorkflowTaskUrl;

		/*$('.tab-content .btn-group').find('button').each(function(i, obj) {
			//console.log('obj');
			//console.log(obj);

			var FlowStepId = $(this).attr('data-flow-step-id');

			if(FlowStepId != '')
				FlowStepId = JSON.parse(FlowStepId);

			activePK = JSON.parse(sessionStorage['active-id-' + me.selectedLib]);
				...
				success: function (response) {
					...
					if (response.length) {
						response.forEach(function (item1) {
							$('#screen_workflow_current_step_'+item1.FlowId).val(JSON.stringify(response));
						});
					}
				}
			});
