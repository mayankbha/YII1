...
var commonApp = function () {
    ...
    this.load = function () {
        ...
        $(document).ready(function () {
			...
			if($('.search-input-inner-wrapper .search-field').attr('data-search')) {
				//console.log('in if');
        ...
        switch(action) {
            ...
            case 'report':
                $('#generate-report-modal')
                    .val({
                        id: $(target).data('reportTemplate'),
                        name: $(target).data('reportName'),
                        simpleSearch: $(target).data('simpleSearch'),
                        multiSearch: $(target).data('multiSearch'),
                        batchQuery: $(target).data('batchQuery')
                    })
                    .modal('show');
				break;
			case 'document':
        ...
        $(document)
			...
			.on('change', '.common-refresh-section-class', function () {
				$('.common-refresh-section-class').each(function(i, obj) {
					...
					active_tab_id = $('.screen-tab.active').attr('data-tab-id');
					section_depth_value = $(this).val();

					var refresh_section_val = $('.refresh-section-class-'+field_identifier).val();

					//alert('field_identifier :: ' + field_identifier);
					//alert('refresh_section :: ' + refresh_section);
					//alert('refresh_section_val :: ' + refresh_section_val);

					//alert('active_tab_id :: ' + active_tab_id);
					//alert('section_depth_value :: ' + section_depth_value);
    ...
    this.init = function () {
		...
		$('.nav-left-group a').click(function (e) {
            ...
            if ($(this).attr('data-action')) {
                if (me.activeMode === 'insert' || me.activeMode === 'edit' ||  me.activeMode === 'copy') {
                    var inputsSelector = $('.tab-content [data-section-lib="' + me.selectedLib + '"]').find('input,select,textarea,keygen').not('.form-control-grid'),
        ...
        if(section_to_refresh != '' && section_depth_value != '' && field_id != '') {
			...
			$.ajax({
				...
				success: function (response) {
					...
					$(tabPlace).find('[data-dependent-field]').dependentField();
					$(tabPlace).find('[readonly][data-krajee-datetimepicker]').each(function () {
						$('#' + $(this).attr('id') + '-datetime').on('show', function(){
							$(this).datetimepicker('hide');
						});
					});

					$(tabPlace).find('[data-relation-field]').each(function () {
						$(this).trigger('custom-focus');
					});

					//$(tabPlace).find('.common-linked-list-field').trigger('change');

					if(me.activeMode == 'insert' || me.activeMode == 'edit') {
						//console.log('change');

						$(tabPlace).find('input.form-control.tt-input').css('background-color', '#fff');
					}

					$(tabPlace).find('#search_extra_param').each(function () {
						...
						if(layout_type == 'TABLE' && mode == 'edit' && readonly == 1) {
							$(tabPlace).find('.add-sub-item').parent().parent().hide();
							$(tabPlace).find('.remove-sub-item').parent().hide();
					...
					$(tabPlace).find('.grid-view').each(function () {
					...
					if (me.activeMode == 'insert' || me.activeMode == 'edit'  || me.activeMode == 'copy') {
						var gridItem = $(tabPlace).find('.grid-view');
					...
					var tabindex = 1;
					$('.tab-content').find('.stats-section, .header-section').each(function () {
					...
					$(tabPlace).find('[tabindex="1"]').trigger('focus');

					if($('.workflow-task-notification').text() != 0 && $('.workflow-task-notification').text != '')
						$('.workflow-task-notification').show();

					if(me.activeMode == 'insert') {
						$(tabPlace).find('.common-refresh-section-class').each(function () {
							if(field_id != '' && section_depth_value != '') {
								var field_val = $('#'+field_id).val();

								if(field_val != '' && field_id == $(this).attr('id'))
									$('#'+field_id).val(section_depth_value);
							}
						});

						setTimeout(function() {
							$.each(serializeArray, function (index, item) {
								//console.log('serializeArray $.each');

								var name = serializeArray[index].name;
								var val = serializeArray[index].value;

								//console.log(name);
								//console.log(val);

								$(tabPlace).find("input[name='"+name+"']").val(val);
								$(tabPlace).find("select[name='"+name+"']").val(val);
								$(tabPlace).find("textarea[name='"+name+"']").val(val);

								$(tabPlace).find('input[id^=dimage_]').each(function (k, v) {
									//console.log('in dimage each');

									var id = $(this).attr('id');

									var image_id_split = id.split('_');

									var temp_name = $(this).attr('name');

									//console.log('id :: ' + id);
									//console.log('temp_name :: ' + temp_name);
									//console.log('image_id_split :: ' + image_id_split[1]);

									if(name == temp_name)
										$('#dimage_show__'+image_id_split[1]).attr('src', 'data:image/jpeg;base64,'+val).show();
								});
							});

							$('.search-input-inner-wrapper .search-field').val('');
						}, 2000);
					}
				},
				error: function () {
		...
		$.ajax({
			...
			success: function (response) {
				...
				if(mode != null && mode == 'edit') {
					...
					$('#document-modal').find('#view-document-list-tbl thead').append(
						'<tr>' +
							'<th>File Name</th>' +
							'<th>Description</th>' +
							'<th>File Size</th>' +
